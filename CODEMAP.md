...
| `extension/` | MV3 browser-udvidelse: "📎 Tilføj til sag" i Gmail/Outlook → **interaktiv popup** (`picker.js`, Shadow DOM: vælg sag(er)/multi/ny-sag/redigér emne+dato+afsender+**indhold**/søg) → CaseBoard (nonce+origin-valideret `message`-modtager m. `targets` i app.js). `datefmt.js` (auto-dato fra ren-tekst-div), `outlook.js`/`gmail.js` (skrab: body = største container ikke emne-elementet, afsender = 1. @ i DOM), `bridge.js` (synker sags-liste + ack), `background.js` (durabel kø). Mailen gemmes som `.html` → **vises inline** (sandboxed iframe, `fileKind='mail'`). |
